...
interface Item {
  ...
  tax_amount?: number;
  total?: number;
  product?: number; // added to support GRNItemsEditor
}
...
const AddGRNItemModal = ({ onSave, onClose, initialData }: Props) => {
  ...
  useEffect(() => {
    const base = formData.unit_price * formData.received_quantity;
  ...
  useEffect(() => {
    if (initialData) {
      const merged = {
        ...formData,
        ...initialData,
        product_id: initialData.product_id || initialData.product || Date.now(),
        received_quantity: initialData.received_quantity ?? 1,
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: ['quantity_ordered', 'received_quantity', 'unit_price', 'discount_percentage', 'tax_rate'].includes(name)
        ? parseFloat(value)
        : value,
    }));
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-900 dark:text-gray-200 rounded-xl shadow-2xl p-6 w-full max-w-2xl">
        <h2 className="text-2xl font-bold mb-6">
          {initialData ? 'Edit Item in GRN' : 'Add Item to GRN'}
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm mb-1">Quantity Ordered</label>
            <input
          ...
          <div>
            <label className="block text-sm mb-1">Received Quantity</label>
            <input
        ...
        <div className="flex justify-end mt-6 space-x-3">
          <button onClick={onClose} className="px-4 py-2 bg-gray-300 hover:bg-gray-400 text-gray-800 rounded">
            Cancel
          </button>
...
export default AddGRNItemModal;
